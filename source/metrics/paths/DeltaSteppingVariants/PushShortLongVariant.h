 ...
 *		fall outside of the current bucket based on long arcs (arcs with weight >= delta) to the last phase.
 */
class PushShortLongVariant {
public:
    /**
            ...
            while (true) {
                ...
                auto question_function = [&active_nodes, &distances, &long_arcs, delta, current_epoch, my_rank, my_rank_cast,
                    ...
                    for (const auto& [target_rank, target_node_id, weight] : out_arcs) {
                        ...
                        const auto target_bucket = EpochCommon::compute_bucket_index(delta, old_distance);
                        if (target_bucket > current_epoch && utility::safe_cast<distance_type>(weight) >= delta) {
                            // This is a long arc
