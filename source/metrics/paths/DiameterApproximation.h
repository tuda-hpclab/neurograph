...
public:
    /**
     * @brief Approximates the diameter of the graph by contracting balls of radius r around randomly
     *		chosen centers and computing the exact diameter of the (much smaller) contracted graph.
     *		Draws the random numbers from a DefaultRandomNumberGenerator that is seeded with the own
     *		MPI rank, so repeated runs choose the same centers.
     *		Must be called on every MPI rank, returns the result only on MPI rank 0.
     * @param graph The graph
     * @param r The radius of the balls that are grown around the centers before they are contracted
     * @return An approximation of the diameter, i.e., the diameter of the contracted graph plus 2 * r
     */
    [[nodiscard]] static distance_type compute_approximation(const DistributedGraph& graph, const distance_type r) {
        ...
        auto random_number_generator = DefaultRandomNumberGenerator{ utility::safe_cast<unsigned int>(my_rank.get_rank()) };

        return compute_approximation(graph, r, random_number_generator);
    }
     ...
     * @param graph The graph
     * @param r The radius of the balls that are grown around the centers before they are contracted
     * @param random_number_generator The generator whose numbers from [0, 1) decide which nodes become centers
     * @return An approximation of the diameter, i.e., the diameter of the contracted graph plus 2 * r
     */
    [[nodiscard]] static distance_type compute_approximation(const DistributedGraph& graph, const distance_type r,
                                                             RandomNumberGenerator& random_number_generator) {
        auto timer_construct_rma_windows = utility::RegionTimer{ "diameter-approximation-construct-rma-windows" };
        ...
        timer_construct_rma_windows.stop();

        auto timer_construct_cluster = utility::RegionTimer{ "diameter-approximation-construct-cluster" };

        for (auto iteration = iteration_type(0); iteration < last_iteration; iteration++) {
            const auto has_center_locally
                = mark_as_centers(number_local_nodes, utility::safe_cast<node_id_type>(number_global_nodes), iteration, random_number_generator, rma_windows);
        ...
        timer_construct_cluster.stop();

        auto timer_construct_graph = utility::RegionTimer{ "diameter-approximation-construct-graph" };

        const auto contracted_graph = contract(graph, rma_windows);

        timer_construct_graph.stop();

        auto timer_delete_rma_windows = utility::RegionTimer{ "diameter-approximation-delete-rma-windows" };
        timer_delete_rma_windows.stop();

        auto timer_small_apsp = utility::RegionTimer{ "diameter-approximation-small-apsp" };

        const auto [_1, _2, approximated_diameter, _3, _4] = AllPairsShortestPath::compute_apsp(contracted_graph);

        timer_small_apsp.stop();

        mpiPP::MPIRegionTimerReport::reduce_and_print("Diameter Approximation");
...
private:
    ...
    [[nodiscard]] static bool mark_as_centers(const node_id_type number_local_nodes, const node_id_type number_global_nodes, const iteration_type iteration,
                                              RandomNumberGenerator& random_number_generator, RMAWindowHolder& rma_windows) {
        const auto my_rank = mpiPP::MPIInfo::get_my_rank();
        ...
        for (auto node_id = node_id_type(0); node_id < number_local_nodes; node_id++) {
            ...
            const auto random_number = random_number_generator.draw();
    ...
    static void perform_growing_step(const DistributedGraph& graph, const distance_type r, const iteration_type iteration, RMAWindowHolder& rma_windows) {
        ...
        const auto generate_question = [my_rank, my_rank_cast, r, iteration, &rma_windows, &graph](const node_id_type node_id) {
            ...
            for (const auto& out_arc : out_arcs) {
                ...
                if (utility::safe_cast<distance_type>(weight) > 2 * r) {
                    continue;
                }
    ...
    [[nodiscard]] static DistributedGraph contract(const DistributedGraph& graph, RMAWindowHolder& rma_windows) {
        ...
        const auto get_node_translation = [number_local_nodes, my_rank, my_rank_cast, &to_correct_locally, &to_correct_globally, &rma_windows]() {
            ...
            for (auto node_id = node_id_type{ 0 }; node_id < number_local_nodes; node_id++) {
                ...
                if (center_rank == my_rank_cast && center_node_id == node_id) {
                    ...
                    anti_node_translation[utility::safe_cast<node_id_type>(number_required_nodes)] = node_id;
        ...
        // Not a structured binding: clang rejects capturing one in a lambda while OpenMP is
        // enabled (fixed in clang 21), and both halves are captured by the lambdas below
        auto translation = get_node_translation();
        auto& node_translation = std::get<0>(translation);
        auto& anti_node_translation = std::get<1>(translation);
        ...
        const auto correct_global_translation = [to_correct_globally, &node_translation, &graph]() {
            ...
            for (const auto node_id : to_correct_globally) {
                ...
                utility::Exception::check(!answers_for_node.empty(), "Some node got no new center answered");
        ...
        const auto produce_out_arcs = [number_required_nodes, number_local_nodes, my_rank, my_rank_cast, &node_translation, &rma_windows, &graph]() {
            // The answers of one questioner come back grouped by the answering rank, not in the order
            // the questions were generated, so each question carries its arc index and each answer
            // echoes it back to match the answer to its arc
            using answer_type_arc_translation = std::pair<arc_id_type, NodeIdentifier>;

            const auto generate_questions_arc_translation = [my_rank_cast, &graph](const node_id_type node_id) {
                using ResultType = mpiPP::comm_patterns::GenerateQuestionsFunction<arc_id_type, node_id_type>::result_type;
                ...
                for (auto arc_id = arc_id_type{ 0 }; arc_id < out_arcs.size(); arc_id++) {
                    const auto& [target_rank, target_node_id, _] = out_arcs[arc_id];
                    targets.emplace_back(target_rank, target_node_id, arc_id);
                }

                return targets;
            };

            const auto generate_answers_arc_translation = [&node_translation](const node_id_type node_id, const arc_id_type arc_id) {
                // This is where the new target is
                const auto& contracted_center = node_translation[node_id];
                return answer_type_arc_translation{ arc_id, contracted_center };
            };

            const auto& answers_arc_translation = mpiPP::comm_patterns::node_to_node_question<arc_id_type, answer_type_arc_translation, node_id_type>(
            ...
            for (auto node_id = node_id_type{ 0 }; node_id < number_local_nodes; node_id++) {
                ...
                utility::Exception::check(answers_for_node.size() == out_arcs.size(), "Some out arc got no target center answered");

                // Restore the arc order via the echoed arc indices
                auto new_target_centers = std::vector<NodeIdentifier>(out_arcs.size(), NodeIdentifier::uninitialized());
                for (const auto& [answered_arc_id, target_center] : answers_for_node) {
                    new_target_centers[answered_arc_id] = target_center;
                }
                ...
                for (auto arc_id = std::size_t{ 0 }; arc_id < out_arcs.size(); arc_id++) {
                    ...
                    const auto& new_center_target = new_target_centers[arc_id];
                    ...
                    const auto new_weight = utility::safe_cast<distance_type>(weight) + source_distance + target_distance;

                    if (new_center_rank == my_rank_cast) {
                        // The source did not move (new_target_rank == old_target_rank possible)
                        new_out_arcs[new_center_node_id][NodeIdentifier{ new_target_rank, new_target_node_id }] = utility::safe_cast<weight_type>(new_weight);
                    } else {
                        // Source moved (new_target_rank == old_target_rank possible)
                        auto new_out_arc = out_arc;
                        new_out_arc.weight = utility::safe_cast<weight_type>(new_weight);
        ...
        const auto produce_in_arcs = [number_required_nodes, my_rank_cast, &new_out_arcs, &graph]() {
            auto new_in_arcs = LoadedArcs{};
            new_in_arcs.resize(number_required_nodes);

            const auto generate_questions_mirror_arcs = [&new_out_arcs, my_rank_cast, number_required_nodes](const node_id_type node_id) {
        ...
        const auto produce_new_nodes = [&graph, &anti_node_translation, number_required_nodes, my_rank_cast]() {
        ...
        return DistributedGraph::construct_graph(produce_new_nodes(), new_in_arcs, new_out_arcs);
    }
};
