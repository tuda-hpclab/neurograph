...
public:
     ...
     */
    [[nodiscard]] static distance_type count_out_arcs(const DistributedGraph& graph) {
        const auto number_local_out_arcs = graph.get_number_local_out_arcs();
        const auto cast = utility::safe_cast<distance_type>(number_local_out_arcs);
        const auto number_total_out_arcs = mpiPP::MPIReductions::reduce_sum(cast);
     ...
     * @return The total number of out arcs
     */
    [[nodiscard]] static distance_type all_count_out_arcs(const DistributedGraph& graph) {
        const auto number_local_out_arcs = graph.get_number_local_out_arcs();
        const auto cast = utility::safe_cast<distance_type>(number_local_out_arcs);
        const auto number_total_out_arcs = mpiPP::MPIReductions::all_reduce_sum(cast);
