...
std::tuple<double, double, double, double> Assortativity::compute_assortativity(const DistributedGraph& graph) {
    const auto average_in_in_degree = AverageDegree::all_compute_average_in_in_degree_by_weights(graph);
    const auto average_in_out_degree = AverageDegree::all_compute_average_in_out_degree_by_weights(graph);
    const auto average_out_in_degree = AverageDegree::all_compute_average_out_in_degree_by_weights(graph);
    const auto average_out_out_degree = AverageDegree::all_compute_average_out_out_degree_by_weights(graph);
    ...
    for (auto node_id = node_id_type{ 0 }; node_id < number_local_nodes; ++node_id) {
        // The source node degrees are constant across the out-arcs, so compute them once per node.
