...
std::array<long double, 14> NetworkMotifs::compute_network_triple_motifs(const DistributedGraph& graph) {
    ...
    auto timer = Timer{ "NetworkMotifs" };

    // Calculate the networkMotifs
    const auto collect_possible_network_motifs_one_node = [my_rank, &graph](const node_id_type node_local_ind) {
        using ResultType = mpiPP::comm_patterns::GenerateQuestionsFunction<ThreeMotifStructure, node_id_type>::result_type;

        const auto out_arcs = graph.get_out_arcs(my_rank, node_local_ind);
        const auto in_arcs = graph.get_in_arcs(my_rank, node_local_ind);
        ...
        for (const auto& [target_rank, target_id, _] : out_arcs) {
            if (target_rank == my_rank && target_id == node_local_ind) {
                continue;
            }
        ...
        for (const auto& [source_rank, source_id, _] : in_arcs) {
            if (source_rank == my_rank && source_id == node_local_ind) {
                continue;
            }
        ...
        for (auto i = std::size_t{ 0 }; i < adjacent_nodes_list.size(); ++i) {
            ...
            const auto& [node_2_exists_outEdge, node_2_exists_inEdge] = arcs_out_in_to_node_outer;
            ...
            for (auto j = std::size_t{ 0 }; j < adjacent_nodes_list.size(); ++j) {
                ...
                const auto& [node_inner, arcsOutIn_to_node_Inner] = adjacent_nodes_list[j];
                const auto& [node_inner_rank, node_inner_id] = node_inner;
                const auto& [node_3_exists_outEdge, node_3_exists_inEdge] = arcsOutIn_to_node_Inner;

                auto motif_struc = ThreeMotifStructure{ .node_3_rank = node_inner_rank, .node_3_local = node_inner_id };

                auto exists_arc_bit_array = 0U;
                exists_arc_bit_array |= node_2_exists_outEdge ? 1U : 0U;
                exists_arc_bit_array |= node_2_exists_inEdge ? 2U : 0U;
                exists_arc_bit_array |= node_3_exists_outEdge ? 4U : 0U;
                exists_arc_bit_array |= node_3_exists_inEdge ? 8U : 0U;
                ...
                this_node_possible_motifs.emplace_back(node_outer.first, node_outer.second, motif_struc);
            }
        }
    ...
    const auto evaluate_correct_network_motifs_one_node = [my_rank, &graph](const node_id_type node_local_ind,
    ...
    timer.time_section("ComputeMotifsCodes");

    // Count the number of motifs locally
    auto motif_type_count = std::array<std::uint64_t, 14>{};

    for (auto node_local_ind = node_id_type{ 0 }; node_local_ind < number_local_nodes; ++node_local_ind) {
        const auto& this_node_motifs_results = three_motif_results.get_answers_of_questioner_node(node_local_ind);
    ...
    timer.time_section("CollectMotifNumber");
    ...
    timer.time_section("CollMotifNumGlobal");
    ...
    timer.time_section("ComputDistribution");
    timer.finish();
...
std::array<long double, 14> NetworkMotifs::compute_network_triple_motifs_sequential(const DistributedGraph& graph) {
    ...
    for (auto current_rank = mpi_rank_type{ 0 }; current_rank < number_ranks; ++current_rank) {
        for (auto current_node = node_id_type{ 0 }; current_node < number_nodes_of_ranks[utility::save_cast<std::size_t>(current_rank)]; ++current_node) {
                ...
                for (; iter_inner != adjacent_nodes.end(); ++iter_inner) {
                    ...
                    if (node_inner_key != node_outer_key) {
                        ...
                        const std::uint16_t exists_arc_bit_array_updated = update_arc_bit_array(
                            graph, utility::save_cast<std::uint16_t>(exists_arc_bit_array), node_outer_key.first, node_outer_key.second, motif_struc.node_3_rank, motif_struc.node_3_local);
...
void NetworkMotifs::ThreeMotifStructure::set_motif_types(const std::vector<unsigned int>& motif_types) {
    for (const auto motif_type : motif_types) {
        ...
        motif_type_bit_array |= utility::save_cast<std::uint16_t>(1U << motif_type);
    }
}
...
void NetworkMotifs::ThreeMotifStructure::unset_all_but_motif_types(const std::vector<unsigned int>& motif_types) {
    auto maintained_motif_types = std::unordered_set<unsigned int>{};
    maintained_motif_types.insert(motif_types.begin(), motif_types.end());

    auto motif_types_to_unset = std::vector<unsigned int>{};
    motif_types_to_unset.reserve(14);

    for (auto motif_type = 1U; motif_type < 14; motif_type++) {
        if (maintained_motif_types.contains(motif_type)) {
...
void NetworkMotifs::ThreeMotifStructure::print_out_complete() const {
    fmt::print("({},{})", node_3_rank, node_3_local);
    print_out();
}
