...
public:
     ...
     * @return The average distance, or 0.0 if the graph has fewer than two nodes
     */
...
private:
    [[nodiscard]] static double compute_local_sum_pair_distances(const DistributedGraph& graph) {
        ...
        auto local_sum = get_summed_distance(graph, my_rank, node_distribution[utility::safe_cast<std::size_t>(my_rank)]);
        ...
        for (auto offset = 1U; offset <= ranks_to_compute; ++offset) {
            ...
            local_sum += get_summed_distance(graph, other_rank, node_distribution[utility::safe_cast<std::size_t>(other_rank)]);
        }
    ...
    [[nodiscard]] static double compute_local_maximum_pair_distances(const DistributedGraph& graph) {
        ...
        auto local_maximum = get_maximum_distance(graph, my_rank, node_distribution[utility::safe_cast<std::size_t>(my_rank)]);
        ...
        for (auto offset = 1U; offset <= ranks_to_compute; ++offset) {
            ...
            local_maximum = std::max(local_maximum, get_maximum_distance(graph, other_rank, node_distribution[utility::safe_cast<std::size_t>(other_rank)]));
        }
    ...
    [[nodiscard]] static double compute_local_minimum_pair_distances(const DistributedGraph& graph) {
        ...
        auto local_minimum = get_minimum_distance(graph, my_rank, node_distribution[utility::safe_cast<std::size_t>(my_rank)]);
        ...
        for (auto offset = 1U; offset <= ranks_to_compute; ++offset) {
            ...
            local_minimum = std::min(local_minimum, get_minimum_distance(graph, other_rank, node_distribution[utility::safe_cast<std::size_t>(other_rank)]));
        }
