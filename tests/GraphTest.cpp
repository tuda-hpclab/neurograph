...
[[nodiscard]] std::pair<LoadedArcs, LoadedArcs> generate_arcs(const std::size_t size, const std::vector<ArcType>& Arcs,
    ...
    for (const auto& [source_rank, source_node_id, target_rank, target_node_id, weight, bidirectional] : Arcs) {
        if (std::cmp_equal(source_rank, my_rank)) {
            ...
            if (target_node_id >= num_nodes_per_rank[utility::save_cast<std::size_t>(target_rank)]) {
        ...
        if (bidirectional && std::cmp_equal(target_rank, my_rank)) {
            if (source_node_id >= num_nodes_per_rank[utility::save_cast<std::size_t>(source_rank)]) {
                spdlog::error("Trying to use non-existent node_id {} in rank {}, arc: ({}, {}, {}, {}, {})", source_node_id, source_rank, source_rank,
                              source_node_id, target_rank, target_node_id, weight);
            }
...
DistributedGraph GraphTest::get_small_four_rank_graph() {
    ...
    for (auto rank = 0U; rank < number_ranks; ++rank) {
        num_nodes_per_rank.push_back(get_positions(utility::save_cast<mpi_rank_type>(rank)).size());
    }

    const auto num_nodes = num_nodes_per_rank[utility::save_cast<std::size_t>(my_rank)];
...
DistributedGraph GraphTest::get_standard_four_rank_graph() {
    ...
    const auto get_in_arcs = [](const mpi_rank_type mpi_rank) {
        ...
        in_arcs.emplace_back(ac{ ce(mpi_rank, 0, 1), ce(mpi_rank, 4, 2), ce(mpi_rank, 5, 9), ce(mpi_rank, 6, 2) });
    ...
    const auto get_out_arcs = [](const mpi_rank_type mpi_rank) {
        ...
        out_arcs.emplace_back(ac{ ce(mpi_rank, 0, 1) });
...
DistributedGraph GraphTest::get_standard_four_rank_graph_on_one_rank() {
    ...
    for (const auto mpi_rank : { 0U, 1U, 2U, 3U }) {
        ...
        in_arcs.emplace_back(ac{ ce(0, 0 + my_offset, 1), ce(0, 4 + my_offset, 2), ce(0, 5 + my_offset, 9), ce(0, 6 + my_offset, 2) });
    ...
    for (const auto mpi_rank : { 0U, 1U, 2U, 3U }) {
        ...
        out_arcs.emplace_back(ac{ ce(0, 0 + my_offset, 1) });
