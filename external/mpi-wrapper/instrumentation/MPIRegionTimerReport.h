...
public:
    ...
    static void reduce_and_print(const std::string_view title, const MPICommunicator& communicator = MPICommunicator::World, std::ostream& output_stream = std::cout) {
        ...
        // Built in a stream of its own, so that neither the flags nor the locale of the caller's
        // stream are touched and neither of them can change the table. The manipulators do what
        // fmt::format("{:<32} {:>14.3f} {:>14.3f}\n", ...) did before, which keeps this header
        // free of {fmt} and the table the same in a build without it (MPI_WRAPPER_NO_FMT).
        auto table = std::ostringstream{};
        table.imbue(std::locale::classic());
        table << title << '\n' << std::string(title.size(), '-') << '\n';
        table << std::left << std::setw(key_width) << "key" << ' ' << std::right << std::setw(time_width) << "avg [ms]" << ' ' << std::setw(time_width) << "max [ms]" << '\n';

        table << std::fixed << std::setprecision(3);
        for (auto index = std::size_t{ 0 }; index < entries.size(); index++) {
            table << std::left << std::setw(key_width) << entries[index].key << ' ';
            table << std::right << std::setw(time_width) << sum_times_ms[index] / number_ranks << ' ' << std::setw(time_width) << max_times_ms[index] << '\n';
        }

        output_stream << table.str();
    }
...
private:
    /** @brief The column width of the key, which is not truncated when it is longer */
    static constexpr int key_width{ 32 };
    /** @brief The column width of each of the two times */
    static constexpr int time_width{ 14 };
