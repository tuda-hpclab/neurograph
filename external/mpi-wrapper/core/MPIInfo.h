...
public:
    ...
    /**
     * @brief Returns the current MPI rank's id as string, zero-padded to the width of the largest
     *      rank, i.e., as "07" in a run with 10 to 99 ranks
     * @exception Throws an Exception if init(...) has not been called before
...
private:
    static void init(const int _number_ranks, const int _my_rank) {
        ...
        // Zero-padded to the width of the largest rank, so that the strings of all ranks are
        // equally long. Padded by hand instead of with fmt::format("{1:0>{0}}", ...), so that this
        // header needs no {fmt} and produces the same string in a build without it.
        const auto num_digits = std::size_t{ utility::num_digits(number_ranks - 1) };
        my_rank_string = std::to_string(my_rank);
        if (my_rank_string.size() < num_digits) {
            my_rank_string.insert(std::size_t{ 0 }, num_digits - my_rank_string.size(), '0');
        }
    }
