...
// Without {fmt} there is no formatter; std::ostream& operator<< above, which this one forwards
// to, stays either way.
#if !defined(MPI_WRAPPER_NO_FMT)
/**
 * @brief Formats an MPICartesianCoordinates through its std::ostream& operator<<, i.e., as
 *      "MPICartesianCoordinates: (1, 2, 3)" respectively "MPICartesianCoordinates: uninitialized"
 */
template <>
