...
[[nodiscard]] inline std::string format_int_with_leading_zeros(const int number, const unsigned int nr_of_digits) {
    ...
    if (text.size() >= nr_of_digits) {
        return text;
    }

    // The sign counts towards the width, so the padding goes behind it.
    const auto sign_length = std::string::size_type{ text.starts_with('-') ? 1U : 0U };
    text.insert(sign_length, std::string(nr_of_digits - text.size(), '0'));
    return text;
}
