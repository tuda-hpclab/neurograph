...
[[nodiscard]] inline std::string make_source_location_key(const std::source_location& location) {
    ...
    return std::string{ file_name } + ':' + std::to_string(location.line());
}
...
/**
 * @brief Formats a value with a fixed number of decimal places, e.g., 1.5 with a precision of 2 as "1.50".
 *      Imbues the classic locale, so that the decimal point does not depend on the program's global locale
 * @tparam T A type that can be written to an output stream. The stream's precision applies to every
 *      value that the type writes, which also formats an aggregate such as StatisticalMeasures
 * @param value The value to format
 * @param precision The number of decimal places
 * @return The formatted value
 */
template <typename T>
[[nodiscard]] std::string format_fixed(const T& value, const int precision) {
...
[[nodiscard]] inline std::string format_duration(const std::chrono::nanoseconds duration) {
    ...
    return format_fixed(static_cast<double>(duration.count()) / divisor, 2) + ' ' + std::string{ unit };
}
...
public:
    ...
    [[nodiscard]] std::string format_report(const std::string_view title, LineFormatter&& line_formatter) const {
        ...
        const auto render = [&](const auto& self, const MapEntry* map_entry, const std::size_t depth) -> void {
            ...
            result += map_entry->first + ": " + line_formatter(map_entry->first, entry.data, parent_data);
