...
public:
    ...
    [[nodiscard]] bool empty() const noexcept {
        // Naming the capacity is redundant, a buffer without storage never holds an element, but it is what
        // relates emptiness to the storage: without it the optimizer cannot tell that a non-empty buffer has
        // a slot to access, and warns that front(), back(), pop() and pop_front() index the null storage of
        // a zero-capacity buffer (-Wnull-dereference). full() states the same relation already.
        return capacity() == 0 || number_elements == 0;
    }
