...
public:
    ...
    /**
     * @brief Renders the id as text: in the format [value: ..., flags: 010...] with the flags in order of
     *      their indices, or as "<name>: <value>" if the traits give the id a name, respectively
     *      "<name>: <unset_text>" if the traits' unset flag is unset.
     *      The one rendering of the ostream output and of the fmt formatter below, so that the two cannot drift apart
     * @return The rendered id
     */
    [[nodiscard]] std::string to_string() const {
        if constexpr (has_name) {
            if constexpr (has_unset_flag) {
                if (!get_flag<Traits::unset_flag>()) {
                    return std::string{ Traits::name } + ": " + std::string{ Traits::unset_text };
                }
            }
            return std::string{ Traits::name } + ": " + std::to_string(static_cast<std::uint64_t>(get_value()));
        } else {
            auto text = "[value: " + std::to_string(static_cast<std::uint64_t>(get_value())) + ", flags: ";
            [this, &text]<std::size_t... Indices>(std::index_sequence<Indices...>) {
                ((text += (this->template get_flag<Indices>() ? '1' : '0')), ...);
    ...
    /**
     * @brief Prints the id to the ostream, identical to to_string()
    ...
    friend std::ostream& operator<<(std::ostream& output_stream, const TaggedID& id) {
        return output_stream << id.to_string();
    }
...
// Without {fmt} the library has no formatter; TaggedID::to_string and operator<< stay either way.
#if !defined(CPP_UTILITIES_NO_FMT)
/**
 * @brief Formats a TaggedID via fmt, identical to TaggedID::to_string(). Without a name in its traits the
 *      format is [value: ..., flags: 010...], with a name it is "<name>: <value>", respectively
 *      "<name>: <unset_text>" if the traits' unset flag is unset
