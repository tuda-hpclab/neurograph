...
public:
     ...
     * @return A non-null pooled pointer, or a non-null separately allocated pointer while the pool is exhausted.
     * @exception std::bad_alloc If fallback allocation fails.
     * @exception std::logic_error If the pool holds a chunk pointer that does not point into its own storage.
     */
    [[nodiscard]] T* get_pointer() {
        if (pointers.empty()) {
            auto* const fallback = allocator_traits::allocate(default_allocator, chunk_size);
            if (fallback == nullptr) {
                throw std::bad_alloc{};
            }
            return fallback;
        ...
        if (ptr == nullptr || memory == nullptr) {
            // Unreachable: a non-empty pool always holds pointers into its own storage. Stating that invariant
            // explicitly keeps the postcondition "never returns nullptr" visible to the optimizer, which would
            // otherwise have to prove it and instead warns about a possible null dereference at the construction
            // site of the calling container (-Wnull-dereference through PoolAllocator).
            throw std::logic_error{ "MemoryPool holds an invalid chunk pointer" };
        }
